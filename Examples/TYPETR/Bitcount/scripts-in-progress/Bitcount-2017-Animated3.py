...
EXPORT_PATH = 'animated2017-2.gif'
  
ROOT_DIR = '/'.join(pagebot.__file__.split('/')[:-2]) + '/'
FONT_DIR = ROOT_DIR + 'fonts/'
FONT_NAME = 'BitcountGrid2-GX.ttf'
...
def makeAnimatiom():
    ...
    for n in range(0, FRAMES, STEP):
        ...
        for cIndex, c in enumerate((c1, c2, c3)):
            ...
            drawGlyphPath(vMasterFont, 'two', S*45, S*19.5, location=location, s=S*0.09, fillColor=c)
            drawGlyphPath(vMasterFont, 'zero', (S+7)*45, S*19.5, location=location, s=S*0.09, fillColor=c)
            drawGlyphPath(vMasterFont, 'one', (S+14)*45, S*19.5, location=location, s=S*0.09, fillColor=c)
            drawGlyphPath(vMasterFont, 'seven', (S+19.2)*45, S*19.5, location=location, s=S*0.09, fillColor=c)
