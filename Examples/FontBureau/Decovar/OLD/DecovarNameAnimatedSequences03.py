...
EXPORT_PATH = '_export/DecovarAnimated_%s_%s.gif'
...
H = S*90
AXIS_FACTOR = 1000
FRAMES = 100 # Number of frames per sequence.
MOVE = FRAMES/80.0
STEP = 1 # 20
...
def makeAnimatiom():
    ...
    for sequence in getProofSequences():   
        for location in getLocations(axes, sequence): # Expand into a sequence of locations. 
            ...
            fill(0)
            fontSize(24)
            text('%s %s' % (location['changed1'], location['changed2']), (100, 20))
        saveImage(EXPORT_PATH % (location['changed1'][0], location['changed2'][0]))
