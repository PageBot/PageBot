...
from pagebot.fonttoolbox.objects.font import Font, getFontPath
...
class BabelString:
    ...
    def textLines(self, w=None, h=None):
        ...
        return self.e.context.textLines(self, w, h)

    def textSize(self, w=None):
        """Calculating the size of text is context dependent.
        This method can only be rendered if self.e.context is defined.
        """
        assert self.e is not None and self.e.context is not None
        if w is None: 
            w = self.e.w # Use width of the referenced element
        return self.e.context.textSize(self, w)

    def textOverflow(self, w=None, h=None):
        """Calculating the overflow text is context dependent.
        This method can only be rendered if self.e.context is defined.
        """
        assert self.e is not None and self.e.context is not None
        if w is None: 
            w = self.e.w # Use width of the referenced element
        if h is None:
            h = self.e.h # Use height of the referenced element
        return self.e.context.textSize(self, w, h)

    def _get_size(self):
        """Not implemented for BabelString, as we need a context in order
        to calculate. Convert to another context string format to get
        rendered textlines.
        """
        raise NotImplementedError
    side = property(_get_size)
    ...
    def _get_type(self):
        """Answers the class name as ID of the class, in case a caller wants to know what
        kind of BabelString this is.
        
        >>> from pagebot.contexts import getContext
        >>> context = getContext('Flat')
        >>> fs = context.newString('ABCD')
        >>> fs.type
        'FlatContext'
        """
    ...
    def fromBabelString(self, pbs):
        """Construct a formatted self.newString() from BabelString instance.
        To be implemented by inheriting BabelString classes.
        """
        raise NotImplementedError

    def asBabelString(self):
        """Construct a formatted BabelString.newString() from self. 
        To be implemented by inheriting BabelString classes."""
        raise NotImplementedError
