...
class DrawBotString(BabelString):
    ...
    def setPrefix(self, prefix):
        """If prefix is None, then ignore. Otherwise replace trailing white space by the prefix
        (which can be an empty string, just to strip the white space).
        """
        if bs.s and prefix is not None:
            while bs.s and bs.s[0] in ' \t\r\n': # Remove trailing spaces, generated by MarkDown
                bs.s = bs.s[1:]
