...
def ru(u, *args, **kwargs):
    ...
    if isinstance(u, (list, tuple)):
        ...
        for uu in u:
            ...
            ruu.append(uu)
        return tuple(ruu)
    #else: # Must be a single unit or value
    u = unit(u, **kwargs)
    if uu is not None:
        uu = uu.r
    return uu
...
def uv(u, *args, **kwargs):
    """Answer the clipped value of `u`. Otherwise use `u`. Convert to *int* if
    whole number.
...
class Pt(Unit):
    ...
    >>> pt('6pt')
    6pt
    >>> u = unit('6pt') # unit function auto-detects matching unit class.
    >>> u
    6pt
    >>> u/2
    3pt
...
def p(v, *args, **kwargs):
    ...
    (10p, 11p, 12p, 13p)
    >>> p(mm(5), pt(24), inch(5)) # Arguments can be a list of other Unit types.
    (1p2.17, 2p, 30p)
    >>> p('10pt', '1"', '1.5"', unit('1.5"')+3) # Arguments can be a list of other Unit types.
    (0p10, 6p, 9p, 27p)
...
def inch(v, *args, **kwargs):
    ...
    if isinstance(v, (tuple, list)):
        ...
        if v.endswith(Inch.UNITC): # "-character
            ...
            u = inch(unit(v), min=minV, max=maxV)
    return u
...
class Inch(Unit):
    ...
    (10", 11", 12", 13")
    >>> inch(mm(5), pt(24), px(5)) # Arguments can be a list of other Unit types.
    (0.20", 0.33", 0.07")
    >>> inch('10pt', '11mm') # Arguments can interprete from strings of other Unit types.
    (0.14", 0.43")
    """
...
class RelativeUnit(Unit):
    ...
    def _get_base(self):
        ...
        (113.385888, 40mm, 20)
        >>> u = unit('5em', base=dict(em=pt(12), perc=pt(50)))
...
def px(v, *args, **kwargs):
    ...
    if isinstance(v, (tuple, list)):
        ...
        if v.endswith(Px.UNIT):
            ...
            u = unit(v, base=base, g=g, min=minV, max=maxV)
...
class Perc(RelativeUnit):
    ...
    >>> u-30+0.51
    70.51%
    >>> unit('66%', base=500).r # Render value towards base unit
    330
    >>> unit('66%', base=mm(500)).r # Render value towards base unit
    330mm
    >>> Perc(1.2) + 1.2
    2.4%
    """
...
def units(v, *args, **kwargs):
    ...
    >>> units('100%', '92%')
    (100%, 92%)
    >>> units('   80  perc  ') # Spaces are trimmed.
    80%
    >>> units('12pt')
    12pt
    >>> units('10"', '2"', inch('12'))
    ...
    if isinstance(v, (list, tuple)):
        ...
        return tuple(uu)
    #else: # Must be a single unit or value
    return unit(v, **kwargs)

def unit(v, maker=None, g=None, base=None, min=None, max=None, default=None):
    ...
    >>> unit('100%')
    100%
    >>> unit('   80  perc  ') # Spaced are trimmed.
    80%
    >>> unit('12pt')
    12pt
    >>> unit('10"')
    10"
    >>> unit('10 inch  ') # Trim any white space.
    10"
    >>> unit('140mm')
    140mm
    >>> unit('30pt')
    ...
    >>> unit(mm(5), default=0)
    5mm
    >>> unit(mm('xyz'), default=pt(13)) # Use default if value cannot be evaluated.
    13pt
    >>> unit(10, maker=p) # All types of makers work: method, name, class
    10p
    >>> unit(10, maker='p') # By name
    10p
    >>> unit(10, maker=P) # By class
    10p
    >>> unit(12) # Default for plain number is pt if no class defined.
    12pt
    >>> unit('SomethingElse') is None
    True
    >>> unit('12pt', 'mm') # Altering maker units converts.
    4.23mm
    >>> unit(mm(12), pt) # Casting to another unit type
    34.02pt
    >>> u1 = unit('10pt')
    >>> u1 is unit(u1) # Creates copy of u1
    False
    ...
    if u is None and default is not None:
        u = unit(default, g=g, base=base, min=min, max=max)
