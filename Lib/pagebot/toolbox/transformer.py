...
def asFormatted(value, default=None, hasFormat=None):
    ...
    if hasFormat is None:
        ...
        # Then remove any trailing zeros (in case of a decimal point)
...
def font2Name(font):
    ...
    if font is not None:
        ...
        if name is None and font.info: # The font may not have been saved yet, no filename exists.
...
def xmlAttrName2PyAttrName(key):
    ...
    prefix `'ns_'`. If an HTML5 attribute *data-xxxx* is used, then
...
def xmlAttr2PyAttr(par_dict, conversions):
    ...
    prefix. If an HTML5 attribute *data-xxxx* is used, then change
    that to *data_xxxx*."""
    pydict = {}
...
def value2Bool(v):
    ...
    The value2Bool method answers the interpreted value of v as boolean. The
    following values (independent of case) interpret as False: ['', '0', 'f',
    'F', 'none', 'false'].  If v is a list or tuple, then it is True if at
    least one element renders to True, so it performs an OR.
