...
class ImageCaption(Element):
    u"""The ImageCaption element is a “normal” container, which contains one (or more)
    Image elements and zero (or more) caption or other elements. This way
    the user can add mulitple Images, a title elements, etc.  The layout of
    the Image elements is defined in the same way as any other layout.
    Conditional rules can be applied (e.g. if the image element changes size),
    or the child elements can be put on fixed positions.
    ...
    (300pt, 400pt)
    >>> page = doc[1]
    >>> e = ImageCaption(imagePath, parent=page, x=0, y=20, w=page.w, h=300)
    ...
    >>> image = e.image
    >>> image.path.endswith(imagePath)
    True
    >>> #TODO: image.size, image.imageSize # Strange difference between local run and Travis
    >>> #((300, 300, 1), (398, 530))
    >>> #((300, 300, 1), (3024, 4032))
    >>> view = doc.getView()
    >>> e.build(view, pt(0, 0))
    ...
    >>> doc = Document(w=w, h=h, autoPages=1, padding=30, originTop=False, context=c)
    >>> page = doc[1]
    >>> e = ImageCaption(imagePath, parent=page, x=0, y=20, w=page.w, h=pt(300))
    >>> image = e.image
    >>> image.path.endswith(imageFilePath)
    True
    >>> #TODO: image.size, image.imageSize # Strange difference between locel run and Travis
    >>> #((300, 300, 1), (398, 530))
    >>> #((300, 300, 1), (3024, 4032))
    >>> # Allow the context to create a new document and page canvas. Normally view does it.
    >>> c.newPage(w, h)
    >>> e.build(doc.getView(), pt(0, 0))
    [Image.build_flat] Not implemented yet
    >>> e.xy
    (0pt, 20pt)
    >>> e.size
    (300pt, 300pt)
    """
    def __init__(self, path=None, style=None, image=None, name=None, title=None, caption=None, clipRect=None,
        ...
        assert path is None or image is None # One or the other or both None.
        ...
        if image is None:
            # Path can be None or non-existing, making Image show gray rectangle of missing image.
            image = Image(path, name='Image', clipRect=clipRect, mask=mask, imo=imo, w=w, h=h,
                conditions=imageConditions, **kwargs) # Default width is leading.
        self.image = image # Property to add to self.elements and set image.parent to self.
        ...
        # Solve the optional conditions defined in title, image and caption.
        self.solve()

    def _get_image(self):
        return self._image # Special element, as there normally is only one image. Use self.elements otherwise.
    def _set_image(self, image):
        self._image = image # Keep last pixel map as self.image attribute. Otherwise use self.elements.
        if image is not None: # Can be None to clear.
            self.appendElement(image) # Add to self.elements and set image.parent to self.
    ...
    def _get_w(self):
        if self._w is None: # Undefined elastic, get size from image
            w = self.image.w
        else:
            w = self._w
    ...
    def _get_h(self):
        if self._h is None: # Undefined elastic, get size from image
            h = self.image.h
        else:
            h = self._h
    ...
    def _get_exists(self):
        u"""Answer the boolean flag if the *self.image.path* image file really
        exists."""
    ...
    def addFilter(self, filter):
        ...
        object in case it doest not exist yet."""
        if self.image is not None:
...
class Image(Element):
    ...
    def __init__(self, path, name=None, w=None, h=None, size=None, z=0, clipRect=None, 
        ...
        if size is not None:
            self.size = size
        else: # Separate settings, to keep proportions if only one it set.
            if w is not None:
                self.w = w
            if h is not None:
                self.h = h
    ...
    def _get_w(self):
        ...
        if not self._w: # Width is undefined
            if self._h and self.ih:
                u = self.iw * self._h.pt / self.ih.pt  # Height is lead, calculate width.
            else:
