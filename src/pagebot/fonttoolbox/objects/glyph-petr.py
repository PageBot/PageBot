...
from pagebot.fonttoolbox.analyzers.pointcontext import PointContext
from pagebot.fonttoolbox.analyzers.point import Point
from pagebot.fonttoolbox.analyzers.segment import Segment
from pagebot.fonttoolbox.analyzers.contour import Contour
...
=======
    ...
    def _drawQuadratic2Cubic(self, p0, p1, p2, path):
        pp0x = p0[0] + (p1[0] - p0[0])*C
        pp0y = p0[1] + (p1[1] - p0[1])*C
        pp1x = p2[0] + (p1[0] - p2[0])*C
        pp1y = p2[1] + (p1[1] - p2[1])*C
        path.curveTo((pp0x, pp0y), (pp1x, pp1y), (p2[0], p2[1]))
>>>>>>> Stashed changes
    ...
    def _get_pointContexts(self):
        if self._pointContexts is None:
            ...
            for cIndex, contour in enumerate(self.contours):
                # Tuples of points -3, -2, -1, 0, 1, 2, 3 contour index
                numPoints = len(contour)
                points = contour.points + contour.points + contour.points
                for n in range(numPoints):
                    pc = PointContext(points[n+numPoints-3:n+numPoints+4], n, cIndex, contour.clockWise, self.name)
