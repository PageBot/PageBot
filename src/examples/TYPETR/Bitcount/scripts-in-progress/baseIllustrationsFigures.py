...
def make_zeros():
    ...
    for cond, slashed, lc, caps in variants:
        ...
        #fs = FormattedString(s, font=FONTS['Single'], fontSize=FS)
        drawString(s, 0, 0, 'Single')
        resetFeatures()

def make_doubleFigures_new():
    s0 = 'ABC0123456789'
    s1 = 'abc0123456789'
    s2 = 'Fraction 01234/56789'
    w = (len(s0)*6+2)*PIX
    ...
    newPage(w, H+PIX)
    openTypeFeatures(smcp=True, frac=False)
    drawGrid(w, H+PIX)
...
make_doubleFigures()
make_zeros()
make_doubleFigures_new()

saveImage('../images/referenceImagesFigures.png', 1)
saveImage('../images/referenceImagesFigures.pdf')