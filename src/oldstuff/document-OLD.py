...
class Galley(Element):
    u"""A Galley can ben be seen as a flow of elements, where the parts can have different widths (some images
    and tables) or responsive width, such as images and formatted text volumes."""
...
class TextBox(Element):
    ...
    def size(self, page, fs):
        self.fs = fs
        # Try to figure out what the height of the text is, before it overflows on the rect.
        # TODO: Needs to be replaced by textSize() as soon as it is available
...
class Image(Element):
    def __init__(self, path, x, y, w=None, h=None, eId=None, s=None, sx=None, sy=None, alpha=1, stroke=None, strokeWidth=None, missingImageFill=NO_COLOR):
        self.x = x
        self.y = y
    ...
    def setPath(self, path):
        ...
        if self.path is not None and os.path.exists(self.path):
            ...
            print '@#+#++#+#', self.w, self.h, self.iw, self.ih
    ...
    def draw(self, page):
        if self.path is None:
            ...
            image(self.path, (self.x/self.sx, self.y/self.sy), self.alpha)
...
class CImage(Image):
    def draw(self, page):
        ...
        print self.sx, self.sy, pw, ph, self.w, self.h, self.iw, self.ih
        print self.sx, self.sy, self.w, self.h, self.iw, self.ih
        if self.path is None:
            self._drawMissingImage(px, py, pw, ph)
        else:
            save()
            scale(self.sx, self.sy)
            image(self.path, (px/self.sx, py/self.sy), self.alpha)
            if self.stroke is not None:
                fill(None)
                setStrokeColor(self.stroke, self.strokeWidth * sx)
                rect(self.x/self.sx, self.y/self.sy, self.w/self.sx, self.h/self.sy)
            restore()
...
class Grid(Element):
    ...
    def draw(self, page):
        ...
        if style.gridStroke is not NO_COLOR:
            ...
            M = 20
            fs = FormattedString('', font='Verdana', align='right', fontSize=M/2, stroke=None, fill=(style.gridStroke, style.gridStroke, style.gridStroke))
            x = style.ml
            index = 0
            y = style.h - style.mt
...
class BaselineGrid(Element):
    ...
    def draw(self, page):
        ...
        while y > style.mb:
            ...
            lineTo((page.w - M, y))
            drawPath() 
            text(fs + `line`, (M-2, y-M*0.65))  
            text(fs + `line`, (page.w - M-4, y-M*0.65))  
...
class Page(object):
    ...
    def image(self, path, x, y, w=None, h=None, eId=None, s=None, sx=None, sy=None, alpha=1, stroke=None, strokeWidth=None):
        e = Image(path, x, self.h - y, w, h, eId, s, sx, sy, alpha, stroke, strokeWidth)
        self.append(e)
            
    def cImage(self, path, cx, cy, cw=None, ch=None, eId=None, s=None, sx=None, sy=None, alpha=1, stroke=None, strokeWidth=None):
        e = CImage(path, cx, cy, cw, ch, eId, s, sx, sy, alpha, stroke, strokeWidth)
        self.append(e)
...
class Template(Page):
    u"""Template is a special kind of Page class. Possible the draw in the same way. Difference is
    that templates cannot contain other templates."""
    ...
    def draw(self, page):
        pass # Templates are supposed to be copied from by Page, not to be drawing themselves.
...
class Composer(object):
    ...
    def typesetFile(self, fileName, page, flowId='main'):
        ...
        # Collect all text in one formatted string, while simulating the page/flow, because
        # we need to keep track on which page/flow nodes results get positioned.   
        self.typesetNode(root, page, tb)
        # Now run through the footnotes and typeset them on the pages where the reference is.
