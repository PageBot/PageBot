...
ML = 7*U # Margin left
MT = 7*U # Margin top
CW = CH = 11*U # Column with and column height. Fit with baseline.
G = U # Generic gutter.
...
H3_TRACK = 0.035 # Tracking as relative factor to font size.
P_TRACK = 0.035
# Language settings
LANGUAGE = 'nl-be'
...
def makeDocument():
    ...
    template1.cTextBox('', 4, 3, 2, 5, flowId2, nextBox=flowId0, nextPage=1, fill=BOX_COLOR)
    ...
    template2.cTextBox('', 4, 3, 2, 5, flowId2, nextBox=flowId0, nextPage=1, fill=BOX_COLOR)
    # Create page number box. Pattern PAGENUMBER is replaced by actual page number.
    ...
    doc = Document(PW, PH, ml=ML, mt=MT, cw=CW, gridStroke=0.8, pages=2, template=template2,
        baselineGrid=BASELINE_GRID, missingImageFill=MISSING_IMAGE_FILL) 
    ...
    doc.newStyle(name='caption', stripWhiteSpace=False, 
        fill=0.2, leading=BASELINE_GRID, fontSize=BODYTEXT_SIZE,
        font=BOOK_ITALIC)
